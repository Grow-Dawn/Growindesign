...
switch ($action) {
    ...
    case 'loginsubmit':
        ...
        if ($userInfo=checkLogin($email, $password)) {
    ...
    case 'vote':
        if (CheckSession()) {
            $itemId = (int) filter_input(INPUT_GET, 'itemId', FILTER_SANITIZE_NUMBER_INT);
            $vote = (strtolower($_GET['vote']) == 'up') ? 1 : 2;
            SaveItemVote($itemId, $vote);
            $item = GetItemById($itemId);
            $comments = GetCommentsWithUsersForItem($itemId);
            $page = 'views/itemdetails.php';
        } else {
            $loginMessage = 'You need to be logged in to vote.';
            $page = 'views/login.php';
        }
        include $page;
        break;
